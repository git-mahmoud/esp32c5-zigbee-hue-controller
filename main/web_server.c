/*
 * web_server.c — Hue Candle HTTP server
 *
 * Features:
 *  - Connection status indicator (paired / not paired)
 *  - Live sliders: lamp updates as you drag
 *  - Brightness and color temperature
 *  - Favorites: apply, save current, delete
 *  - Android shortcuts: /cmd?on  /cmd?off  /cmd?fav=N
 *  - Pair button with visual feedback
 */
...
static esp_err_t h_fav_put(httpd_req_t *req)
{
    ...
    if (last) apply_favorite((uint8_t)atoi(last + 1));
    httpd_resp_send(req, NULL, 0);
    return ESP_OK;
}
...
{
    const char *last = strrchr(req->uri, '/');
    if (last) favorites_delete((uint8_t)atoi(last + 1));
    httpd_resp_send(req, NULL, 0);
    return ESP_OK;
}

/* /cmd — Android home screen shortcuts */
static esp_err_t h_cmd(httpd_req_t *req)
{
    char query[64] = {0};
    httpd_req_get_url_query_str(req, query, sizeof(query));

    if (strncmp(query, "on", 2) == 0)       { set_on_off(true); }
    else if (strncmp(query, "off", 3) == 0) { set_on_off(false); }
    else if (strncmp(query, "pair", 4) == 0) {
        /* Trigger Touchlink commissioning */
        ...
        if (fv) apply_favorite((uint8_t)atoi(fv + 4));
    }
